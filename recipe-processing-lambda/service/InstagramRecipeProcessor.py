...
class InstagramRecipeProcessor:
    ...
    # -----------------------------
    # 1️⃣ Invoke InstagramMediaProcessor Lambda
    # -----------------------------
    def invoke_media_processor(self, url: str) -> dict:
        ...
        if response.get("FunctionError"):
            ...
            logger.error(f"Instagram media processor Lambda failed: {error}")
            raise RuntimeError(f"Instagram media processor Lambda failed: {error}")

        if payload.get("statusCode") != 200:
            error = json.loads(payload.get("body", "{}")).get("error", "Unknown error")
            logger.error(f"Instagram media processor returned error: {error}")
            raise RuntimeError(f"Instagram media processor returned error: {error}")
